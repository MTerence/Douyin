//
//  DYMineVerticalCollectionHeader.m
//  Douyin
//
...
@interface DYMineVerticalCollectionHeader ()
...
@implementation DYMineVerticalCollectionHeader
...
- (void)layoutSubviews{
    //    self.topbackgroundImageView.frame = self.bounds;
}
...
- (LXDSegmentControl *)segmanetControl{
    if (_segmanetControl == nil) {
        ...
        //使用配置对象创建分栏控制器
        _segmanetControl = [LXDSegmentControl segmentControlWithFrame:CGRectMake(0, self.freeShootBtn.bottom + DY_SCALE_WIDTH(20), self.width, self.height - self.freeShootBtn.bottom - DY_SCALE_WIDTH(20)) configuration: configuration delegate: self];
    }
